...
/* ── Avatar palette — deterministic colour from name ── */
function avatarStyle(name: string): { bg: string; fg: string } {
  const p: [string, string][] = [
    ["#EFF6FF", "#2563EB"], ["#F0FDF4", "#16A34A"], ["#FFF7ED", "#EA580C"],
    ["#FAF5FF", "#7C3AED"], ["#FFF1F2", "#E11D48"], ["#F0FDFA", "#0D9488"],
...
/** scoreCls — maps hybrid score to CSS modifier (hi / mid / lo) */
function scoreCls(s: number) {
  return s >= 80 ? "hi" : s >= 60 ? "mid" : "lo";
}

/** relDate — human-readable relative date */
function relDate(str?: string | null): string {
  if (!str) return "";
  const d = Math.round((Date.now() - new Date(str).getTime()) / 86_400_000);
  if (d === 0) return "Today";
...
interface CandidateCardProps {
  candidate: Candidate & {
    ...
    dept?:          string | null;
    /** Populated by hasRedFlag() — passed from parent list render */
    red_flags?:     string[];
    /** Hybrid match label — passed from scoring engine */
    match_type?:    string;
  };
  isSelected: boolean;
  onClick:    () => void;
  /** Rank position — passed when shown in ⭐ Top 10 view */
  rank?:      number;
}

/**
 * CandidateCard
 * Mirrors .cs-cc HTML structure from document index 17 exactly.
 *
 * CSS classes live in CandidateScreeningInterface global CSS:
 *   cs-cc, cs-cc--on, cs-cc--flagged, cs-cc__av, cs-cc__body,
 *   cs-cc__nm-row, cs-cc__nm, cs-cc__flag, cs-cc__rank,
 *   cs-cc__email, cs-cc__role, cs-cc__meta, cs-cc__flags-row,
 *   cs-cc__flag-tag, cs-cc__right, cs-cc__score, cs-cc__denom,
 *   cs-cc__match-pill, cs-cc__match--strong/partial/weak,
 *   cs-pill, ftab--danger, ftab--star
 */
const CandidateCard: React.FC<CandidateCardProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Body */}
      <div className="cs-cc__body">
        {/* Name row — name + optional red-flag icon + optional rank badge */}
        <div className="cs-cc__nm-row">
          <span className="cs-cc__nm">{candidate.name}</span>
          {hasFlags && (
            <span className="cs-cc__flag" title={redFlags!.join(", ")}>
              🚩
            </span>
          )}
          {rank && <span className="cs-cc__rank">#{rank}</span>}
        </div>
        ...
        {/* Status pill + optional match-type mini pill */}
        <div className="cs-cc__meta">
          <span className={`cs-pill ${st.cls}`}>{st.label}</span>
          {matchType && (
            <span
        ...
        {/* Red-flag detail tags — only shown in 🚩 Red Flags filter view */}
        {hasFlags && (
          <div className="cs-cc__flags-row">
            {redFlags!.slice(0, 2).map((f) => (
              <span key={f} className="cs-cc__flag-tag">
                {f}
              </span>
            ))}
            {redFlags!.length > 2 && (
              <span className="cs-cc__flag-tag">
                +{redFlags!.length - 2} more
              </span>
            )}
          </div>
        )}
